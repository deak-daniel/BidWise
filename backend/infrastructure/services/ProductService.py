...
class ProductService:
    def get_products():
        return db.query(ProductBdo).options(selectinload(ProductBdo.fxRate)).all()
    
    def get_product_id(id: int):
        return db.query(ProductBdo).options(selectinload(ProductBdo.fxRate)).filter(ProductBdo.id == id).first()
